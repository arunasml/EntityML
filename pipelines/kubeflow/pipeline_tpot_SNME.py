...
def scaling_tpot(
    ...
    def task(input_entity, sema):
        with sema:
            sample_fraction = 1.0
            entity_partition = input_entity
            print(f"Running {entity_partition} ...")
            ...
            tpot.export(output_dir.path, entity_partition)
 
    # main func without using Pool
    concurrency = 7
    ...
    for i in range(int(entity_partition_input)):
        p = multiprocessing.Process(target = task, args=(f"entity_{i}",sema))
        p.start()
        processes.append(p)
...
vertex_exp_name = f"case_{exp_case}-group{exp}-{prediction_type}"
...
INPUT_ENTITY = f"{exp}"
DISPLAY_NAME = f"scaling-tpot-{vertex_exp_name}"
...
def pipeline(
    ...
    task = (
        scaling_tpot(input_url=input_url, entity_partition_input=entity_partition_input)
        .set_cpu_limit("32")
